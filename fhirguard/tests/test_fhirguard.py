from fhirguard.fhirguard import FHIRGuard, Metadata
from fhirguard_core.resources import Patient, Coding, CodeableConcept, BackboneElement, OperationOutcomeIssue

import pytest

@pytest.fixture
def guard():
    return FHIRGuard("tests/definitions/hl7.fhir.r4.core")
...
def test_patient_happy(guard: FHIRGuard):
    resource = Patient.construct(
        ...
        link=[BackboneElement.construct(
...
def test_patient_happy_example(guard: FHIRGuard):
    resource = Patient.construct(
        id="123",
        active=True,
        name=[{"family": "Simpson", "given": ["Homer"]}],
        gender="male",
        maritalStatus=Coding.construct(code="M"),
        contact=BackboneElement.construct(
            name={
                "family": "Simpson",
                "given": ["Marge"],
